...
async function determineState(lastCheckHit: number | undefined, userId: string, mangaId: string, lastCheckEpoch: number, currentEpoch: number): Promise<DetermineStateResponse> {
  try {
    ...
    if(!lastCheckHit || (currentEpoch - lastCheckHit) > Duration.SECONDS(5)) { // if we haven't been checking a bunch of series quickly, this may be a regular series view load, so tell it to fetch data
