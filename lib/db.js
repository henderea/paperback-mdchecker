...
async function listUsers() {
  const result = await query('select user_id from user_id', [], 'array');
...
async function getUserUpdateCount(userId) {
  const result = await query('select count(0) as count from user_manga where user_id = $1 and last_update > last_check', [userId]);
  if(result.rowCount <= 0) {
    return -1;
  }
  return result.rows[0].count;
